...
namespace GreenOnions.MiraiApiHttp
{
    ...
    public partial class FriendMessage : IMiraiHttpMessageHandler<IFriendMessageEventArgs>
    {
        public async Task HandleMessageAsync(IMiraiHttpSession session, IFriendMessageEventArgs e)
        {
            ...
            int inMsgId = (e.Chain[0] as SourceMessage).Id;

            if (e.Chain.Length > 1)  //普通消息
            {
                //MessageHandler.HandleMesage(e.Chain.ToOnionsMessages(), e.Sender.Id, outMsgs =>
                //{
                ...
                //        await MessageHandler.HandleMesage(e.Chain, e.Sender,

...
namespace GreenOnions.NT.OneBot
{
    internal abstract class Program
    {
        public static async Task Main(string[] args)
        {
            ...
            Console.WriteLine("欢迎使用葱葱机器人NT + OneBot");
            ...
            {
                Console.WriteLine("不存在OneBot配置(appsettings.json)，尝试创建默认配置");

                var assm = Assembly.GetExecutingAssembly();
                File.WriteAllBytes("appsettings.json", Resource.DefaultSettings);

                Console.WriteLine("已创建OneBot配置(appsettings.json)，请修改配置后按回车继续");
                Console.ReadLine();
            }
